...
class BudgetController:
    ...
    def get_this_month_expenses(self, user_id: int) -> list[tuple[int, int, int, str]]:
        """This function returns all the expenses of the user by their id and the current month as a list of tuples."""
        return self.db.select("expenses", "*", f"WHERE user_id = {user_id} AND strftime('%m', date) = strftime('%m', 'now')")

    def get_remaining_budget(self, user_id: int) -> int:
        """This function returns the remaining budget of the user by their id."""
        return self.db.select("budgeting", "remaining_budget", f"WHERE user_id = {user_id}")[0][0]

    def get_monthly_budget(self, user_id: int) -> int:
        """This function returns the monthly budget of the user by their id."""
        return self.db.select("budgeting", "monthly_budget", f"WHERE user_id = {user_id}")[0][0]